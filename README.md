...
| `/receive all\|smart\|mention\|auto` | Per-chat receive mode (admin, groups only). `all` = reply to every message, `smart` = always reply when `@`-mentioned and let the agent decide whether to chime in on un-mentioned messages (it may stay silent; those messages still become context), `mention` = require `@`, `auto` = follow the global default, no argument = show status |
