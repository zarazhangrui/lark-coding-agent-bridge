...
<<<<<<< HEAD
- **流式回复**：用户态文字在聊天里更新；原始工具调用和 reasoning 默认隐藏，需要排障时再打开调试模式。
=======
...
`/config` 可以控制 run 细节在飞书 / Lark 里展示多少：

- `clean`（默认）：只显示用户态文字和最小处理中状态。原始 `Read` / `Bash` / `Write` / `Edit` 事件以及 reasoning 不进聊天。
- `progress`：显示用户态文字和粗粒度阶段，例如规划、内部步骤、整理回复；命令、工具里的文件路径、输出和 diff 不进聊天。
- `debug`：显示完整 reasoning 和工具调用面板，适合维护者排查 bridge 或 agent 行为。

agent 事件和工具结果仍会写入本地日志 / session 状态；输出模式只影响聊天里渲染什么。
=======
## 回复展示与 COT
