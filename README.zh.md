...
把飞书 / Lark 消息和本地 Claude Code、Codex CLI 或 OpenCode agent 打通的轻量 bot。用一条命令启动，扫码绑定 PersonalAgent 应用，然后在飞书里和本机编程助手对话，让它读图、处理文件、改代码。
...
- 在飞书私聊直接发消息，或在群里 `@bot`，把任务转给本机 Claude Code / Codex CLI / OpenCode。
...
- 本机至少安装并登录一个 agent：
  ...
  - OpenCode：`opencode`，安装方式：`npm i -g opencode-ai@latest`（也可通过 Homebrew / Scoop 等安装；见 https://opencode.ai）
...
`profile use <name>` 会切换后续默认启动使用的 profile。需要同时跑 Claude / Codex / OpenCode 多个 bot、连接多套 PersonalAgent 应用，或做脚本化部署时，再使用这些 profile 管理命令：
...
| `workspace` | `acceptEdits` | `workspace-write` | `build` | yes（与 `full` 行为一致；OpenCode 没有 workspace-write 中间档） |
| `read-only` | `plan` | `read-only` | `plan` | no（权限自动拒绝） |

旧版 `sandbox` 字段仍可读取。bridge 保存 profile 后，会把该设置迁移为 canonical `permissions`。

OpenCode 特有说明：`/config` 中的模型选择器通过运行 `opencode models` 动态填充；可用模型取决于你配置的 provider，会随时间变化。OpenCode 会话通过 `--session <id>` 恢复（类似 Claude，使用 session id 而非 last-message id）。
