...
- 本机至少安装并登录一个 agent：
  ...
  - Kimi Code CLI：`kimi`，安装说明：https://www.kimi.com/kimicode
...
|---|---|---|---|
| `full` | `bypassPermissions` | `danger-full-access` | 由 bridge 强制执行 |
| `workspace` | `acceptEdits` | `workspace-write` | 由 bridge 强制执行 |
| `read-only` | `plan` | `read-only` | 由 bridge 强制执行 |

Kimi 的 prompt 模式不接受 `--yolo`、`--auto` 等原生权限参数，因此由 bridge 自行强制执行访问限制。旧版 `sandbox` 字段仍可读取。bridge 保存 profile 后，会把该设置迁移为 canonical `permissions`。
