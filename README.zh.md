...
- **项目控制台**：bot 进群自动置顶一张菜单卡（`/menu` 随时调出），点按钮就能切换 / 新建 / 添加项目——新建和添加都用目录浏览器点选路径，全程不用打命令
- **图片 / 文件**：直接发给 bot，Claude 会读本地下载的文件路径
- **卡片按钮**：`/menu` `/help` `/ws list` `/status` 返回交互卡片，点按钮直接操作
...
| 命令 | 作用 |
|---|---|
| `/menu` | 弹出并置顶「项目控制台」精简卡片（切换 / 新建 / 添加项目、状态一键直达） |
| `/new` `/reset` | 清空当前 chat 的会话 |
| `/cd <path>` | 切换工作目录（会重置 session） |
| `/ws list` | 列所有命名工作空间（卡片 + 按钮） |
| `/ws new` | 用目录浏览器新建项目（建目录 + 存工作空间 + 切换） |
| `/ws add` | 用目录浏览器把已有目录添加为工作空间 |
