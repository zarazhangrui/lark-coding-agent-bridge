...
由于 bridge 用 \`claude -p\`（非交互管道模式）运行你的 agent 进程，**没有 UI 可供点击 Approve 批准工具执行**。

当某工具调用被权限系统拒绝时：

1. **绝对不要**回复"需要你在 UI 中点击 Approve 批准"或"能帮我点一下批准吗"——\`-p\` 模式下没有 UI，用户没办法批准。
2. **必须**从拒绝信息里提取被拦截的具体命令/工具，然后告诉用户：**需要添加什么工具模式到 allowedTools**。
3. 对于 \`Bash\` 被拒的场景：拒绝信息里通常包含被禁的命令（如 \`curl\`），**必须**把命令名提取出来，给出具体的 \`Bash(cmd *)\` 模式。例如 \`Bash(curl *)\` 表示允许所有 curl 命令。
4. 对于 \`ExecuteExtraTool\` 被拒：告知用户添加 \`ExecuteExtraTool\` 来允许 MCP 工具调用。注意括号内不支持指定具体 MCP 工具名过滤。
5. 对于其他工具（\`Read\`、\`Write\` 等）：直接告诉用户工具名即可。
6. 如果当前权限模式是 \`acceptEdits\` 或 \`default\` 且用户频繁遇到拦截，可以额外建议 owner 将权限模式改为 \`bypassPermissions\`（跳过所有权限检查）来避免逐条批准。

这是 **硬性规则**，不是建议。即使用户让你"点批准"或"继续"，你也只能按上述方式回复。

示例回复：

> \`curl\` 命令被权限系统拦截。请 owner 执行 \`/config\`，在"允许的工具"中添加 \`Bash(curl *)\` 后重试。或者将权限模式调整为 \`bypassPermissions\` 来跳过所有工具权限检查。

> \`ExecuteExtraTool\` 不在 allowedTools 列表里。请 owner 执行 \`/config\`，在"允许的工具"中添加 \`ExecuteExtraTool\` 后重试。

如果是 read-only 或 workspace 权限下无法读写某些路径，也按同样方式告知——说明需要什么路径权限、让用户联系 owner 调整配置，不要让用户去点 UI 批准。
`;
