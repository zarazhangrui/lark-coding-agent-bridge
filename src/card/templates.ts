...
export interface StatusInfo {
    ...
    balance?: string;
    /** Allowed tools string from config. */
    allowedTools?: string;
}
...
export function statusCard(info: StatusInfo): object {
    ...
    return shell('📊 当前状态', [
        divMd(
            [
                `🧭 **scope**: ${info.chatMode === 'topic' ? `\`${escapeCode(info.scope)}\` _（话题独立 session）_` : `\`${escapeCode(info.scope)}\``}`,
                `🧩 **profile**: \`${escapeMd(info.profileName)}\``,
                `📁 **cwd**: ${info.cwd ? `\`${escapeCode(info.cwd)}\`` : '(未设置)'}`,
                `🔗 **session**: ${info.sessionId ? `\`${info.sessionId.slice(0, 8)}…\`${info.sessionStale ? ' ⚠️ 旧 cwd，下一条会新建' : ''}` : info.emptySessionText ?? '(无)'}`,
                ...
                ...(info.activeScopes && info.activeScopes.length > 0 ? [`🏃 **active scopes**: \`${info.activeScopes.map((scope) => `\`${escapeCode(scope)}\``).join(', ')}\``] : []),
                ...(info.activeCommentScopes && info.activeCommentScopes.length > 0
                    ...
                    : []),
                `🚦 **queue**: \`${info.queue ? `${info.queue.active}/${info.queue.cap} active, ${info.queue.waiting} waiting` : 'unknown'}\``,
...
export function resumeCard(cwd: string, entries: ResumeEntry[]): object {
    ...
    if (entries.length === 0) {
        ...
        entries.forEach((e, i) => {
            ...
            elements.push(divMd(`**${i + 1}.** ${escapeMd(e.preview)}${marker}\n\`${displayId.slice(0, 8)}…\` · ${e.relTime} · ${escapeMd(detail)}`));
            elements.push(
                actions([
                    {
