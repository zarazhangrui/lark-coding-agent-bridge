...
export async function ensureSecretsGetterWrapper(
  ...
  if (platform === 'win32') {
    ...
    // Clear any read-only attribute on an existing wrapper first, otherwise the
    // atomic rename below cannot replace it (Windows refuses to overwrite a
    // read-only file).
    await chmod(wrapperPath, 0o600).catch(() => {});
    await writeFileAtomic(wrapperPath, content, { mode: 0o600 });
    // lark-cli's exec-provider security audit treats any writable file as
    // world-writable on Windows (Go maps a non-read-only file to mode 0666 and
    // rejects it). Mark the wrapper read-only so Go reports 0444 and the audit
    // passes; the wrapper only needs to be executed, never rewritten in place.
    await chmod(wrapperPath, 0o400).catch(() => {});
    return wrapperPath;
  }
