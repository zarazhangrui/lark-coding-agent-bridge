...
function isValidAgentEntry(entry: SessionCatalogEntry): boolean {
  ...
  if (entry.agentId === 'antigravity') return Boolean(entry.threadId) && !entry.sessionId;
...
function assertAgentIdentity(input: UpsertSessionCatalogInput): void {
  ...
  if (!input.threadId || input.sessionId) {
    const label = input.agentId === 'codex' ? 'Codex' : 'Antigravity';
    throw new Error(`${label} catalog entries require threadId and must not include sessionId`);
  }
}
