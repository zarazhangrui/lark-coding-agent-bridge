...
**常见问题诊断要点**:
- 如果用户反馈"群聊中不 @ 就收不到消息",检查日志中是否所有 intake 记录都带 mentionedBot: true。如果是,说明飞书开放平台的"接收群内所有消息"权限未开启(这是平台层面的设置,不是 bridge 配置问题)。
- 如果日志中完全没有某些消息的 intake 记录,说明 bridge 根本没收到这些消息,可能是飞书 API 层面的过滤或权限问题。
...
## 建议下一步
1-3 条具体可执行的动作(检查 X / 重启 Y / 等待 Z 之类)。如果诊断出是"接收群内所有消息"权限问题,明确告知需要在飞书开放平台后台(open.feishu.cn)的"机器人"设置中开启该权限。
